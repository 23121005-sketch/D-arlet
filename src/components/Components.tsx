...
export const Navbar = () => {
  ...
  const baseClass =
    "hover:text-brand-gold transition-colors duration-200 text-white font-serif tracking-wide";
  return (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <div className="flex-shrink-0 flex items-center">
            <h1 className="text-2xl font-serif text-brand-gold font-bold">
              Arlet's Restaurant
            </h1>
          </div>
...
export const Chatbot = () => {
  ...
  const [messages, setMessages] = useState<{ text: string; isBot: boolean }[]>([
    {
      text: "¡Hola! Bienvenido a Arlet's Restaurant. ¿En qué puedo ayudarte?",
      isBot: true,
    },
  ]);
  const handleOption = (option: string) => {
    ...
    setTimeout(() => {
      ...
      switch (option) {
        case "¿Hacen delivery?":
          response =
            "¡Sí! Hacemos delivery a toda la ciudad. Puedes hacer pedidos al WhatsApp: +51 987654321.";
          break;
        ...
        case "¿Hacen reservas?":
          response =
            "Sí, puedes reservar directamente desde nuestra sección de contacto o llamándonos.";
          break;
        case "Ver carta":
          response =
            "Puedes ver nuestra deliciosa carta en la sección 'Menú' de esta web.";
          break;
        default:
        ...
        <div className="bg-white rounded-xl shadow-2xl w-80 sm:w-96 overflow-hidden flex flex-col h-[500px] border border-gray-200">
          ...
          <div className="p-4 bg-white border-t space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={() => handleOption("¿Hacen delivery?")}
              ...
              <button
                onClick={() => handleOption("Horario de atención")}
                className="text-xs bg-gray-100 hover:bg-gray-200 p-2 rounded text-left"
              >
                {" "}
                Horario
              ...
              <button
                onClick={() => handleOption("¿Hacen reservas?")}
                className="text-xs bg-gray-100 hover:bg-gray-200 p-2 rounded text-left"
              >
                Reservas
              </button>
              <button
                onClick={() => handleOption("Ver carta")}
                className="text-xs bg-gray-100 hover:bg-gray-200 p-2 rounded text-left"
              >
                {" "}
                Carta
            ...
            <button
              ...
              className="w-full mt-2 bg-green-600 hover:bg-green-700 text-white py-2 rounded flex justify-center items-center gap-2 text-sm"
            >
...
export const ProtectedRoute: React.FC<{
  ...
  console.log(
    "🎯 ¿Rol en allowedRoles?",
    user?.role ? allowedRoles.includes(user.role) : "No hay rol"
  );
  ...
  if (!allowedRoles.includes(user.role!)) {
    console.log(
      "🚫 ProtectedRoute: Acceso denegado. Rol:",
      user.role,
      "no está en",
      allowedRoles
    );
    alert(
      `Acceso denegado. Tu rol (${user.role}) no tiene permiso para esta sección.`
    );
        ...
        {items.map((item, index) => (
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-8 text-white">
              ...
              <span className="inline-block bg-brand-gold text-brand-dark px-4 py-1 rounded-full font-bold">
                S/ {item.price}
              </span>
            </div>
          </div>
