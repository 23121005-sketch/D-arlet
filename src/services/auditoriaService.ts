...
export const auditoriaService = {
  ...
  async obtenerHistorial(limit: number = 10): Promise<Auditoria[]> {
    try {
      ...
      return (data || []).map(item => {
        ...
        const emp = Array.isArray(item.empleados) ? item.empleados[0] : item.empleados;
        ...
        return {
          ...
          tipo: (item.accion.startsWith('MANUAL_') ? 'manual' : 'automático') as 'manual' | 'automático'
        };
      });
    } catch (error) {
  ...
  async obtenerHistorialCompleto(filters: {
    ...
    try {
      ...
      if (filters.fechaInicio) query = query.gte('created_at', `${filters.fechaInicio}T00:00:00`);
      if (filters.fechaFin) query = query.lte('created_at', `${filters.fechaFin}T23:59:59`);
      ...
      if (filters.usuario) {
        result = result.filter(r => r.empleado_nombre?.toLowerCase().includes(filters.usuario!.toLowerCase()));
      }
      if (filters.rol) {
  ...
  async registrarCreacionEmpleado(empleado: any): Promise<boolean> {
    return this.registrarAccion('CREAR_EMPLEADO', 'empleados', empleado.id, { nuevo_empleado: empleado.nombre });
  },

  async registrarActualizacionEmpleado(empleadoId: string, cambios: any): Promise<boolean> {
    return this.registrarAccion('ACTUALIZAR_EMPLEADO', 'empleados', empleadoId, { cambios });
  },

  async registrarEliminacionEmpleado(empleadoId: string, empleadoInfo: any): Promise<boolean> {
    return this.registrarAccion('ELIMINAR_EMPLEADO', 'empleados', empleadoId, { empleado_eliminado: empleadoInfo.nombre });
  },

  async registrarCambioEstado(empleadoId: string, nombre: string, estadoAnterior: string, estadoNuevo: string): Promise<boolean> {
    return this.registrarAccion('CAMBIAR_ESTADO', 'empleados', empleadoId, { empleado: nombre, cambio: `${estadoAnterior} → ${estadoNuevo}` });
  },

  // NUEVOS MÉTODOS PARA RESERVAS Y PEDIDOS
  async registrarReserva(accion: 'CREAR' | 'EDITAR' | 'ELIMINAR' | 'ESTADO', id: string | number, info: any): Promise<boolean> {
    return this.registrarAccion(`${accion}_RESERVA`, 'reservas', String(id), info);
  },

  async registrarPedido(accion: 'CREAR' | 'EDITAR' | 'ELIMINAR' | 'ESTADO' | 'COCINA', id: string, info: any): Promise<boolean> {
  ...
  async obtenerEstadisticas() {
    try {
      ...
      const { count: totalCount } = await supabase.from('auditoria').select('*', { count: 'exact', head: true });
      const { count: hoyCount } = await supabase.from('auditoria')
  ...
  exportarAExcel(datos: Auditoria[]) {
    ...
    const headers = ['Fecha y Hora', 'Usuario', 'Rol', 'Modulo', 'Accion', 'Descripcion Detallada', 'Referencia ID'];
    ...
    link.href = url;
    link.setAttribute('download', `bitacora_arlet_${new Date().toISOString().split('T')[0]}.csv`);
