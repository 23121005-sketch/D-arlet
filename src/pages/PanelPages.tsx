...
import {
  ...
  LayoutGrid, CalendarDays, Map, Bell, Lock, Filter, ArrowRight, Edit3, Download, Info, MessageSquareText, Send, FileCheck
} from 'lucide-react';
import {
...
const PanelLayout: React.FC<{ title: string, children: React.ReactNode, active: string, user: any }> = ({ title, children, active, user }) => {
  ...
  const navItems = [
    ...
    { id: 'delivery', label: 'Pedidos', icon: <Truck size={20} />, path: '/delivery', roles: ['admin', 'delivery'] },
    ...
    { id: 'reclamaciones', label: 'Reclamos', icon: <MessageSquareText size={20} />, path: '/admin-reclamos', roles: ['admin'] },
        ...
        <div className="p-6 border-b border-gray-800">
          ...
          <div className="mt-6 flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-[10px] bg-brand-gold text-brand-dark px-2 rounded-full font-black uppercase tracking-tighter">{user?.role}</span>
            </div>
          </div>
        </div>
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          {navItems.map((item) => (
            <button key={item.id} onClick={() => navigate(item.path)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${active === item.id ? 'bg-brand-gold text-brand-dark font-black shadow-lg shadow-yellow-900/20' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
              {item.icon} <span>{item.label}</span>
            </button>
          ))}
        </nav>
        <div className="p-4 border-t border-gray-800">
          <button onClick={logout} className="flex items-center gap-3 text-red-400 hover:bg-red-900/20 w-full px-4 py-3 rounded-xl font-bold transition-all">
...
const VistaCalendario: React.FC<{ reservas: ReservaLocal[] }> = ({ reservas }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 overflow-x-auto mb-6">
...
const HistorialAvanzadoSection: React.FC<{ 
    ...
    const filteredLogs = historial.filter(log => {
        const matchUser = log.empleado_nombre?.toLowerCase().includes(filters.usuario.toLowerCase());
        const matchRol = filters.rol ? log.empleado_rol === filters.rol : true;
        const matchModulo = filters.modulo ? log.tabla_afectada === filters.modulo : true;
        const matchDate = filters.fecha ? log.created_at.startsWith(filters.fecha) : true;
    ...
    const getActionBadge = (accion: string) => {
        ...
        if (accion.includes('CREAR')) colors = "bg-green-50 text-green-700 border-green-200";
        if (accion.includes('ACTUALIZAR') || accion.includes('EDITAR')) colors = "bg-blue-50 text-blue-700 border-blue-200";
        if (accion.includes('ELIMINAR') || accion.includes('CANCELAR')) colors = "bg-red-50 text-red-700 border-red-200";
        if (accion.includes('CAMBIO_ESTADO')) colors = "bg-orange-50 text-orange-700 border-orange-200";
        if (accion.includes('LOGIN') || accion.includes('ACCESO')) colors = "bg-indigo-50 text-indigo-700 border-indigo-200";
        
        return <span className={`px-2.5 py-1 rounded-full text-[9px] font-black uppercase border tracking-wider ${colors}`}>{text}</span>;
    };

    const getModuloBadge = (modulo: string) => {
        const map: any = {
            'empleados': { label: 'Empleados', icon: <Users size={12}/>, color: 'text-purple-600 bg-purple-50' },
            'pedidos': { label: 'Pedidos', icon: <Package size={12}/>, color: 'text-blue-600 bg-blue-50' },
            'reservas': { label: 'Reservas', icon: <Calendar size={12}/>, color: 'text-amber-600 bg-amber-50' },
            'auditoria': { label: 'Sistema', icon: <Shield size={12}/>, color: 'text-slate-600 bg-slate-50' }
        };
        const config = map[modulo] || { label: modulo, icon: <Info size={12}/>, color: 'text-gray-600 bg-gray-50' };
        return (
            <div className={`flex items-center gap-1.5 px-2 py-1 rounded-lg font-bold text-[10px] ${config.color}`}>
    ...
    return (
        <div className="bg-white p-8 rounded-[48px] shadow-sm border border-gray-100 mb-8 animate-fadeIn">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-10 px-4">
                <div>
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-2xl font-black text-gray-800 tracking-tighter uppercase">Bitácora de Operaciones</h3>
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mt-1">Control de auditoría y trazabilidad en tiempo real</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="text-right hidden sm:block">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Actividad de Hoy</p>
                        <p className="text-xl font-black text-brand-dark">{estadisticas?.hoy || 0}</p>
                    </div>
                    <button onClick={onRefresh} className="p-4 bg-gray-50 text-gray-400 rounded-2xl hover:bg-brand-dark hover:text-white transition-all shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8 bg-gray-50/50 p-6 rounded-[32px] border border-gray-100">
                <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-2">Usuario</label>
                    <div className="relative group">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-brand-gold transition-colors" size={14} />
                        <input 
                            type="text" 
                ...
                <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-2">Módulo</label>
                    <select 
                ...
                <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-2">Rol Usuario</label>
                    <select 
                        className="w-full px-4 py-3 bg-white border border-gray-100 rounded-xl text-xs font-bold outline-none focus:ring-4 focus:ring-brand-gold/10 appearance-none cursor-pointer"
                        value={filters.rol}
                ...
                <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-2">Fecha Evento</label>
                    <input 
                        type="date" 
            ...
            <div className="overflow-x-auto rounded-[32px] border border-gray-100 shadow-sm">
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {loading ? (
                            <tr><td colSpan={6} className="py-20 text-center text-gray-400 font-bold uppercase tracking-widest">Actualizando registros...</td></tr>
                        ) : filteredLogs.map((log) => {
                            const date = new Date(log.created_at);
                            return (
                                <tr key={log.id} className="hover:bg-gray-50/80 transition-all group">
                                    <td className="px-6 py-5">
                                        <p className="text-[11px] font-black text-gray-800">{date.toLocaleTimeString('es-PE', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</p>
                                        <p className="text-[9px] font-bold text-gray-400 uppercase mt-0.5">{date.toLocaleDateString('es-PE', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
                                    </td>
                                    <td className="px-6 py-5">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="text-xs font-black text-gray-800 uppercase tracking-tighter">{log.empleado_nombre}</p>
                                                <p className="text-[9px] font-bold text-gray-400 uppercase leading-none">{log.empleado_rol || 'sistema'}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-5">
                                    ...
                                    <td className="px-6 py-5">
                                        <p className="text-[11px] text-gray-600 font-medium leading-relaxed max-w-xs line-clamp-2 italic">
                                            {log.detalles?.empleado_eliminado ? `Se eliminó al empleado: ${log.detalles.empleado_eliminado}` : 
                                             log.detalles?.nuevo_empleado ? `Registro de nuevo personal: ${log.detalles.nuevo_empleado}` :
                                             log.detalles?.cambio ? `Cambio de estado: ${log.detalles.cambio}` :
                                             JSON.stringify(log.detalles).replace(/{|}|"/g, '').slice(0, 100) || 'Sin detalles adicionales'}
                                        </p>
                                    </td>
                                    <td className="px-6 py-5 text-center">
                                        <span className="text-[9px] font-mono text-gray-300 font-black tracking-tighter">
                                            {log.registro_id ? `REF-${log.registro_id.slice(-6).toUpperCase()}` : '---'}
                                        </span>
                                    </td>
                                </tr>
                            );
                            ...
                            <tr>
                                <td colSpan={6} className="py-24 text-center">
                                    <div className="flex flex-col items-center opacity-20">
                                        <Activity size={64}/>
                                        <p className="mt-4 font-black uppercase text-[10px] tracking-widest">No se encontraron registros para los filtros aplicados</p>
                                    </div>
                                </td>
                            </tr>
                        )}
            ...
            <div className="mt-6 flex justify-between items-center px-4">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Página 1 de 1 (Mostrando {filteredLogs.length} registros)</p>
                <div className="flex gap-2">
                    <button disabled className="px-4 py-2 border rounded-xl text-[10px] font-black uppercase opacity-30 cursor-not-allowed">Anterior</button>
                    <button disabled className="px-4 py-2 border rounded-xl text-[10px] font-black uppercase opacity-30 cursor-not-allowed">Siguiente</button>
                </div>
            </div>
        </div>
    );
...
export const ReservationPanel = () => {
  ...
  const handleStatusChange = async (id: string | number, newStatus: string) => {
    ...
    try {
        const { error } = await supabase.from('reservas').update({ estado: dbStatus }).eq('id', id);
        if (!error) {
            await auditoriaService.registrarReserva('ESTADO', id, { nuevo_estado: newStatus });
        }
        fetchReservations();
  ...
  const handleDelete = async (id: string | number) => {
    try {
        ...
        if (!error) {
            await auditoriaService.registrarReserva('ELIMINAR', id, { cliente: resToDelete?.customerName });
        }
        fetchReservations();
  ...
  const handleGuardarReserva = async (datosReserva: any) => {
    ...
    if (editingReserva) {
        ...
        await auditoriaService.registrarReserva('EDITAR', editingReserva.id, { cliente: resDB.cliente_nombre });
    } else {
        const { data, error } = await supabase.from('reservas').insert(resDB).select();
        if (error) {
            if (error.code === '23505') {
                throw new Error('La mesa seleccionada ya se encuentra ocupada para esa fecha y hora.');
            }
        ...
        if (data?.[0]) {
            await auditoriaService.registrarReserva('CREAR', data[0].id, { cliente: resDB.cliente_nombre });
        }
    }
...
export const DeliveryPanel = () => {
  ...
  useEffect(() => {
    const listosParaReparto = pedidos.filter(p => p.estado === 'listo_para_reparto');
  ...
  const handleStatus = async (id: string, next: string) => {
    try {
        ...
        await auditoriaService.registrarPedido('ESTADO', id, { nuevo_estado: next });
        await cargarData();
  ...
  const filtered = pedidos.filter(p => {
      ...
      const orderDate = p.created_at ? new Date(p.created_at).toLocaleDateString('sv-SE') : '';
  ...
  const columns = [
      ...
      { id: 'listo_para_reparto', label: 'Listos para Recojo', color: 'border-t-orange-500' },
      ...
      {showReadyAlert && (
        <div className="fixed bottom-10 right-10 z-[100] animate-bounce-in">
           <div className="bg-brand-dark border-2 border-brand-gold shadow-2xl rounded-3xl p-6 flex items-center gap-5 max-w-sm">
              ...
              <div>
                 <p className="text-brand-gold font-black text-sm uppercase tracking-tighter leading-tight">El pedido está listo para recojo</p>
                 <p className="text-white text-[10px] font-bold uppercase tracking-widest mt-1">Cliente: {readyOrderName}</p>
                 <p className="text-gray-400 text-[9px] mt-1 italic">Notificación oficial de Cocina.</p>
              </div>
              <button onClick={() => setShowReadyAlert(false)} className="text-white/30 hover:text-white p-1">
                 <X size={18} />
              </button>
           </div>
        </div>
      ...
      <div className="bg-white p-8 rounded-[56px] shadow-sm border border-gray-100 mb-12 flex justify-between items-center animate-fadeIn">
        ...
        <div className="flex items-center gap-4">
            {!audioEnabled && (
                ...
                >
                  <VolumeX size={18} />
                  <span className="text-[10px] font-black uppercase tracking-widest">Activar Sonido Reparto</span>
                </button>
            )}
            {!isAdmin && (
...
const OrderCard: React.FC<{ 
    ...
    const statusConfig: any = {
        ...
        en_preparacion: { label: 'En Cocina', color: 'bg-yellow-100 text-yellow-700', next: null, nextLabel: null, editable: false },
        listo_para_reparto: { label: 'Listo Recojo', color: 'bg-orange-100 text-orange-700', next: 'en_camino', nextLabel: 'Iniciar Reparto', editable: false },
    ...
    const cfg = statusConfig[pedido.estado] || { label: pedido.estado, color: 'bg-gray-100', next: null };
...
export const PanelCocina = () => {
  ...
  useEffect(() => {
    const comandasNuevas = pedidos.filter(p => {
        const kState = kitchenStates[p.id] || (p.estado === 'en_camino' || p.estado === 'listo_para_reparto' ? 'listo' : 'pendiente');
  ...
  const moverAPreparacion = (id: string) => {
    ...
    auditoriaService.registrarPedido('ESTADO', id, { evento: 'INICIO DE PREPARACIÓN EN COCINA' });
  };
  ...
  const marcarComoListo = async (id: string) => {
      try {
          ...
          await auditoriaService.registrarPedido('COCINA', id, { evento: 'PEDIDO LISTO PARA RECOJO' });
      ...
      {showNewOrderVisual && (
          <div className="fixed top-24 right-10 z-[100] animate-bounce-in">
             <div className="bg-brand-dark border-2 border-brand-gold shadow-2xl rounded-2xl p-6 flex items-center gap-4">
                ...
                <div>
                   <p className="text-brand-gold font-black text-sm uppercase tracking-widest">¡Nuevo Pedido!</p>
                   <p className="text-white text-[10px] font-black uppercase opacity-60 mt-0.5">Tienes una orden nueva en cola</p>
                </div>
                <button onClick={() => setShowNewOrderVisual(false)} className="text-white/20 hover:text-white p-1">
                   <X size={20} />
                </button>
             </div>
          </div>
                    ...
                    {getPedidosPorSeccion(sec.id).map(p => (
                        <div key={p.id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-all animate-scaleIn">
                            ...
                            <div className="pt-2">
                                ...
                                {!canModifyKitchen && (sec.id === 'pendiente' || sec.id === 'preparando') && (
                                    <div className="p-3 bg-gray-50 border border-dashed border-gray-200 rounded-2xl text-center">
                                        <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest italic">Solo lectura (Rol Admin)</p>
                                    </div>
                                )}
                            </div>
                        </div>
              ...
              <tr key={pedido.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4">
                    <span className={`px-3 py-1 text-xs font-bold rounded-full border ${
                        ...
                        pedido.estado === 'listo_para_reparto' ? 'bg-orange-50 text-orange-700 border-orange-200' :
                        pedido.estado === 'en_camino' ? 'bg-purple-50 text-purple-700 border-purple-200' :
...
export const AdminPanel = () => {
  ...
  const fetchRealStats = async () => {
    try {
      ...
      const { count: delPen } = await supabase.from('pedidos').select('*', { count: 'exact', head: true }).in('estado', ['pendiente', 'en_preparacion', 'listo_para_reparto', 'en_camino']);
      setCounts({
  ...
  useEffect(() => {
    ...
    const resSub = supabase.channel('stats-res')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'reservas' }, () => {
        fetchRealStats();
        loadHistorial();
      }).subscribe();

    const pedSub = supabase.channel('stats-ped')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'pedidos' }, () => { 
        fetchRealStats(); 
        loadHistorial();
      }).subscribe();

    const auditSub = supabase.channel('audit-live')
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
        ...
        <div className="relative group bg-white p-7 rounded-[40px] shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300">
          ...
          <div className="mt-8 flex items-center gap-2 text-xs font-black text-blue-600">
            <CheckCircle size={14} /> <span className="uppercase tracking-widest">Pedidos Totales</span>
          </div>
        </div>
...
export const AdminReclamacionesPanel = () => {
    ...
    const handleEstado = async (id: string, nuevo: string) => {
        ...
        if (reclamoActual?.estado === 'resuelto') {
            alert('Este expediente ya está RESUELTO y cerrado legalmente. No puede ser modificado.');
            return;
        }

        try {
            const { error } = await supabase.from('reclamaciones').update({ estado: nuevo, updated_at: new Date().toISOString() }).eq('id', id);
            if (error) throw error;
            
            await auditoriaService.registrarAccion('GESTION_RECLAMACION', 'reclamaciones', id, { 
            ...
            loadData();
            if (selReclamo?.id === id) setSelReclamo(prev => prev ? {...prev, estado: nuevo as any} : null);
        } catch (e) { 
    ...
    const handleResponder = async () => {
        ...
        try {
            ...
            if (!PUBLIC_KEY) {
                throw new Error('Configuración de EmailJS incompleta en el código fuente.');
            }
            ...
            await auditoriaService.registrarAccion('RESPUESTA_RECLAMACION_ENVIADA_GMAIL', 'reclamaciones', selReclamo.id, { 
                respondido_por: user?.name,
                email_notificado: selReclamo.cliente_email,
                respuesta: respuesta
            });
            
            alert('¡ÉXITO! La respuesta ha sido enviada al Gmail del cliente y el expediente se ha cerrado legalmente.');
            setRespuesta('');
            setSelReclamo(null);
            loadData();
            ...
            alert(`ERROR: No se pudo enviar la respuesta oficial. Detalles: ${e.message}`);
        } finally {
            setEnviando(false);
        }
    };

    if (user?.role !== 'admin') return <div className="p-20 text-center font-black text-gray-300 uppercase tracking-widest">Acceso Denegado</div>;

    const getStatusColor = (estado: string) => {
        switch(estado) {
            case 'pendiente': return 'bg-rose-50 text-rose-700 border-rose-200';
    ...
    return (
        <PanelLayout title="Libro de Reclamaciones Virtual" active="reclamaciones" user={user}>
            <div className="bg-white rounded-[40px] shadow-sm border border-gray-100 overflow-hidden animate-fadeIn">
                <div className="p-8 border-b bg-gray-50/50 flex justify-between items-center">
                    <div>
                        <h3 className="text-lg font-black text-gray-800 uppercase tracking-widest">Expedientes de Consumidores</h3>
                        <p className="text-[10px] text-gray-400 font-bold mt-1 uppercase tracking-widest">Cumplimiento Normativo - Puerto Maldonado</p>
                    </div>
                    <button onClick={loadData} className="p-3 bg-white border border-gray-200 rounded-2xl hover:bg-brand-dark hover:text-white transition-all shadow-sm">
                        <RefreshCw size={18} className={loading ? 'animate-spin' : ''} />
                    </button>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {loading ? (
                                <tr><td colSpan={5} className="py-24 text-center text-gray-400 font-black uppercase tracking-widest animate-pulse">Consultando registros legales...</td></tr>
                            ) : reclamaciones.length === 0 ? (
                                <tr><td colSpan={5} className="py-24 text-center text-gray-300 font-bold italic">No existen reclamos o quejas registradas.</td></tr>
                            ) : reclamaciones.map(r => (
                                <tr key={r.id} className="hover:bg-gray-50 transition-all group">
                                    <td className="px-8 py-6">
                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-tighter">ID: #{r.id.slice(-6).toUpperCase()}</p>
                                        <p className="text-xs font-bold text-gray-600 mt-1">{new Date(r.fecha).toLocaleDateString('es-PE', { day: '2-digit', month: 'long', year: 'numeric' })}</p>
                                    </td>
                                    <td className="px-8 py-6">
                                        <p className="text-xs font-black text-gray-800 uppercase leading-none">{r.cliente_nombre}</p>
                                        <p className="text-[9px] text-gray-400 font-bold mt-1.5 uppercase tracking-tighter">DNI: {r.cliente_dni}</p>
                                    </td>
                                    <td className="px-8 py-6">
                                        <div className="flex flex-col gap-1">
                                            <span className={`px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-widest border w-fit ${r.tipo === 'reclamacion' ? 'bg-rose-50 text-rose-700 border-rose-100' : 'bg-amber-50 text-amber-700 border-amber-100'}`}>
                                                {r.tipo}
                                            </span>
                                            <p className="text-[9px] text-gray-400 font-black uppercase tracking-widest">{r.tipo_bien}</p>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border w-fit ${getStatusColor(r.estado)}`}>
                                            <div className={`w-1.5 h-1.5 rounded-full ${r.estado === 'resuelto' ? 'bg-green-500' : 'animate-pulse bg-current'}`}></div>
                                            <span className="text-[10px] font-black uppercase tracking-tighter">{r.estado}</span>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6 text-center">
                                        <button onClick={() => setSelReclamo(r)} className="p-3 bg-gray-100 text-gray-400 rounded-2xl hover:bg-brand-dark hover:text-white transition-all shadow-sm">
                                            <Eye size={18}/>
                                        </button>
                                    </td>
                                </tr>
            ...
            {selReclamo && (
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-brand-dark/95 backdrop-blur-md overflow-y-auto" onClick={() => setSelReclamo(null)}>
                    <div className="bg-white rounded-[48px] max-w-4xl w-full shadow-2xl overflow-hidden animate-scaleIn" onClick={e => e.stopPropagation()}>
                        <div className="bg-brand-dark p-10 flex justify-between items-start relative overflow-hidden">
                            ...
                            <div className="relative z-10">
                                <h3 className="text-3xl font-serif font-bold text-brand-gold">Expediente Oficial</h3>
                                <p className="text-white/40 font-mono text-[10px] mt-2 tracking-[0.3em] uppercase">Hoja de Reclamación Virtual #{selReclamo.id.slice(-8).toUpperCase()}</p>
                            </div>
                            <button onClick={() => setSelReclamo(null)} className="text-white/40 hover:text-white p-3 bg-white/5 rounded-full relative z-10 transition-all hover:rotate-90"><X size={24}/></button>
                        </div>
                        
                        <div className="p-12 space-y-10 overflow-y-auto max-h-[70vh] custom-scrollbar">
                            <div className="grid grid-cols-2 gap-10">
                                <div className="space-y-4">
                                    <h4 className="text-[10px] font-black text-brand-gold uppercase tracking-[0.2em] border-b pb-2">1. Información del Reclamante</h4>
                                    <div>
                                        <p className="text-xl font-black text-gray-800 uppercase">{selReclamo.cliente_nombre}</p>
                                        <p className="text-sm font-bold text-gray-400 mt-1 uppercase">DNI: {selReclamo.cliente_dni}</p>
                                    </div>
                                    <div className="space-y-2 bg-gray-50 p-6 rounded-3xl border border-gray-100">
                                        <p className="text-sm font-black text-blue-600 flex items-center gap-2"><Phone size={14}/> {selReclamo.cliente_telefono}</p>
                                        <p className="text-sm font-bold text-gray-600 flex items-center gap-2"><Mail size={14}/> {selReclamo.cliente_email}</p>
                                        <p className="text-xs text-gray-400 font-medium italic"><MapPin size={12} className="inline"/> {selReclamo.cliente_direccion}</p>
                                    </div>
                                </div>
                                <div className="space-y-4">
                                    <h4 className="text-[10px] font-black text-brand-gold uppercase tracking-[0.2em] border-b pb-2">2. Identificación del Bien</h4>
                                    <div className="bg-brand-dark p-8 rounded-[40px] shadow-2xl relative overflow-hidden group">
                                        ...
                                        <p className="text-[9px] font-black text-brand-gold/50 uppercase mb-2 tracking-[0.2em]">Bien Contratado:</p>
                                        <p className="text-lg font-black text-white leading-tight uppercase tracking-tighter">{selReclamo.descripcion_bien}</p>
                                        <div className="mt-4 flex justify-between items-center">
                                            <span className="text-[9px] font-black text-brand-dark bg-brand-gold px-3 py-1 rounded-full uppercase">{selReclamo.tipo_bien}</span>
                                            {selReclamo.monto_reclamado && <span className="text-2xl font-black text-brand-gold">S/ {Number(selReclamo.monto_reclamado).toFixed(2)}</span>}
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-8">
                                <h4 className="text-[10px] font-black text-brand-gold uppercase tracking-[0.4em] border-b pb-2">3. Detalle de Incidencia</h4>
                                <div className="grid md:grid-cols-2 gap-8">
                                    <div className="p-8 bg-gray-50 rounded-[40px] border border-gray-100 relative">
                                        <div className="absolute -top-3 left-8 bg-brand-dark text-brand-gold px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest">El Hecho</div>
                                        <p className="text-sm text-gray-700 leading-relaxed italic font-medium">"{selReclamo.detalle}"</p>
                                    </div>
                                    <div className="p-8 bg-amber-50 rounded-[40px] border border-amber-100 relative">
                                        <div className="absolute -top-3 left-8 bg-amber-600 text-white px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest">Pedido Concreto</div>
                                        <p className="text-sm text-amber-900 font-black leading-relaxed italic">"{selReclamo.pedido_consumidor}"</p>
                                    </div>
                                </div>
                            </div>

                            <div className="space-y-6 pt-6 border-t border-gray-100">
                                <h4 className="text-[10px] font-black text-brand-gold uppercase tracking-[0.4em] mb-4">4. Resolución Administrativa</h4>
                                ...
                                {selReclamo.estado !== 'resuelto' ? (
                                    <>
                                        <div className="flex flex-wrap gap-4 mb-6">
                                            <button 
                                                ...
                                                className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${selReclamo.estado === 'pendiente' ? 'bg-rose-500 text-white shadow-lg scale-105' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}
                                            >
                                                Pendiente
                                            </button>
                                            <button 
                                                onClick={() => handleEstado(selReclamo.id, 'en proceso')}
                                                // Fixed typo: compared against 'en_proceso' instead of 'en proceso'
                                                className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${selReclamo.estado === 'en_proceso' ? 'bg-amber-500 text-white shadow-lg scale-105' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}
                                            >
                                                En Proceso
                                            </button>
                                        </div>
                                        ...
                                        <div className="bg-gray-50 p-8 rounded-[40px] border-2 border-dashed border-gray-200 group focus-within:border-brand-gold transition-colors">
                                            <div className="flex items-center justify-between mb-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="p-2 bg-brand-dark text-brand-gold rounded-xl shadow-lg"><Send size={16}/></div>
                                                    <h4 className="text-[10px] font-black text-gray-700 uppercase tracking-widest">Respuesta Formal al Consumidor</h4>
                                                </div>
                                            ...
                                            <div className="flex justify-end mt-6">
                                                ...
                                                >
                                                    {enviando ? <RefreshCw className="animate-spin" size={16}/> : <FileCheck size={16}/>}
                                                    {enviando ? 'ENVIANDO GMAIL...' : 'ENVIAR RESPUESTA Y CERRAR'}
                                                </button>
                                            </div>
                                        </div>
                                    </>
                                    ...
                                    <div className="p-10 bg-green-50 rounded-[40px] border border-green-200">
                                        <div className="flex items-center gap-3 mb-4">
                                            ...
                                            <h4 className="text-lg font-black text-green-800 uppercase tracking-tighter">Expediente Resuelto</h4>
                                        </div>
                                        <div className="bg-white p-6 rounded-3xl border border-green-100 shadow-sm">
                                            <p className="text-[10px] font-black text-gray-400 uppercase mb-2 tracking-widest">Respuesta Enviada:</p>
                                            <p className="text-sm text-gray-700 italic font-medium leading-relaxed">"{selReclamo.respuesta_empresa}"</p>
                                        </div>
                                        <p className="text-[9px] text-green-600 font-bold mt-4 uppercase tracking-[0.2em] text-center">Este expediente ya fue resuelto.</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="p-10 bg-gray-50 border-t border-gray-100 flex justify-end">
                            <button onClick={() => setSelReclamo(null)} className="px-12 py-4 bg-brand-dark text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-black transition-all shadow-lg">Cerrar Visualización</button>
                        </div>
                    </div>
                </div>
            )}
