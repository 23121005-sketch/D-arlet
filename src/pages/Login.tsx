...
export const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (bloqueado) {
      setError(
        `Sistema bloqueado. Espera ${formatTiempoRestante(tiempoRestante)}`
      );
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (empleadoError || !empleado || empleado.estado !== "activo") {
        ...
        setError(
          empleado?.estado !== "activo" && empleado
            ? "Cuenta inactiva"
      ...
      <div className="min-h-screen flex items-center justify-center bg-black/80 backdrop-blur-sm">
        <div className="bg-white p-12 rounded-2xl shadow-2xl max-w-md w-full text-center">
          ...
          <p className="text-sm text-gray-500 italic">
            Por seguridad, el tiempo de espera aumenta tras varios fallos.
          </p>
        </div>
      </div>
    );
      ...
      <div className="relative z-10 bg-white p-10 rounded-2xl shadow-2xl max-w-md w-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <input
            ...
            className="w-full px-4 py-3 border rounded-lg outline-none focus:ring-2 focus:ring-brand-gold"
          ...
          <button
            type="submit"
            className="w-full bg-brand-dark text-white font-bold py-3 rounded-lg hover:bg-black transition-all shadow-lg"
            disabled={loading}
          >
